...
struct DS4Mmap {
    ...
    bool open_ro(const std::string & path, std::string & err) {
        if (!map.open(path, err)) return false;
        addr = map.data();
        len = map.size();
        return true;
    }
...
bool load_deepseek4_gguf_partial(const std::string & path,
    ...
    if (fast_managed) {
        // Unified/managed buffer: copy weights from the mapped file in parallel
        // at memory bandwidth, then synchronize so CPU writes are visible to GPU.
        ...
        auto worker = [&]() {
            ...
            while ((i = next.fetch_add(1)) < allocs.size()) {
                ...
                if (a.file_offset > mmap.len || a.file_size > mmap.len - a.file_offset) {
                    read_ok = false;
                    return;
                }
                std::memcpy(dst,
                            static_cast<const uint8_t *>(mmap.addr) + a.file_offset,
                            a.file_size);
            }
        };
